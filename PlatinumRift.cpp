...
const int PLATINUM_RIFT_MINOR_VERSION = 7;
//-------------------------------------------------------------------------
...
struct Earth
{
    ...
    string getPodPurchaseCommand()
    {
                ...
                if(zone[i].ownerId==-1 )
                    {
                        ...
                        result = result + to_string(zone[i].id);
                        amount--;
                       if(!amount)i=zone.size();
                       }
            }
        }

        if(amount) // ownerId with  platinum under demand
        {
            for(int i=0; i<zone.size() ; i++)
            {
                if(zone[i].platinum && zone[i].ownerId==my_id && zone[i].hasMyPod(my_id)<3)
                    {
                        if(!result.empty()) result=result +" 1 ";
                        else result = "1 ";
                        amount--;
                        result = result + to_string(zone[i].id);
                        if(amount<1)i=zone.size();
                    }
            }
        }
        ...
        if(amount) // ownerId with  platinum under demand
        {
            for(int i=0; i<zone.size() ; i++)
            {
                if(zone[i].platinum && zone[i].ownerId==my_id && zone[i].hasMyPod(my_id)<zone[i].platinum)
                    {
                        if(amount > zone[i].platinum)
                        {
                            int ti=zone[i].platinum-zone[i].hasMyPod(my_id);
                            if(!result.empty()) result=result +" "+to_string(ti)+" ";
                            else result = to_string(ti)+" ";
                            amount= amount-ti;
                        }
                        else
                        {
        ...
        if(amount) // ownerId and under demand
        {
            for(int i=0; i<zone.size() ; i++)
            {
                if(zone[i].ownerId==my_id && zone[i].hasMyPod(my_id)<zone[i].demand)
                    {
                        if(amount > (zone[i].demand+demand_adj))
                        {
                            int ti=zone[i].demand-zone[i].hasMyPod(my_id);
                            if(!result.empty()) result=result +" "+to_string(ti)+" ";
                            else result = to_string(ti)+" ";
                            amount= amount-ti;
                        }
    ...
    string getMovementCommand()
    {
        ...
        for(int i=0; i<zone.size(); i++)
        {
            if(zone[i].hasMyPod(my_id))
            {
             ...
             if(temp_flag && !zone[i].platinum && zone[i].demand)
             { zone[i].demand =0; }
             else if(temp_flag && zone[i].platinum && zone[i].demand != zone[i].platinum)
             { zone[i].demand = zone[i].platinum; }
